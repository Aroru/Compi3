...
private boolean verificarVariables() {
    while (p.token == 208) { // 'new'
        p = p.sig;

        String dataType = obtenerTipoDato();
        if (dataType == null) return false;

        if (!verificarIdentificador(dataType)) return false;

    }
    return true;
}

private boolean verificarIdentificador(String dataType) {
    if (p.token == 100) { // Suponemos que este token representa un identificador
        ...
        int declarationLine = tablaSimbolos.getDeclarationLine(identifierName);
        if (declarationLine != -1) {
            System.out.println("Error: La variable " + identifierName + " ya ha sido declarada en la línea " + declarationLine);
            return false;
        }
        ...
        if (p.token == 124) { // Suponemos que este token representa ','
            p = p.sig;
            return verificarIdentificador(dataType);
        } else if (p.token == 125) { // Suponemos que este token representa ';'
            p = p.sig;
            return true;
        } else {
            System.out.println("Se espera un punto y coma o una coma después del identificador en la línea " + p.region);
            return false;
        }
    } else {
...
private String obtenerTipoDato() {
    switch(p.token) {
        case 209: p = p.sig; return "float";
        case 213: p = p.sig; return "string";
        case 210: p = p.sig; return "int";
        case 215: p = p.sig; return "bool";
        default:
            System.out.println("Se espera un tipo de variable en la línea " + p.region);
            return null;
    }
}
    ...
    private boolean Statements() {
        ...
        } else if (p.token == 100) { //declaracion de variable
            p = p.sig;
            if (p.token == 123) { 
                p = p.sig;
